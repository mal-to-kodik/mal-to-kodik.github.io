...
async function getIframeSrc() {
	...
	const iframeSrc = queryParams.get('iframe-src');
	if (iframeSrc) {
		const iframeUrlPrefix = 'https://kodik.info/';
		result = iframeSrc.startsWith(iframeUrlPrefix) ? iframeSrc : iframeUrlPrefix + iframeSrc;
	} else if (anime) {
		document.querySelector('title').textContent = anime;
		const translation = queryParams.get('translation');
		const iframeSrc = await getIframeSrcForAnime(anime, translation);
		if (iframeSrc) {
