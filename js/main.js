const ANIME_ID_REGEXP = /^(?<id>\d+)|https:\/\/myanimelist\.net\/anime\/(?<id>\d+).*$/g
...
async function getIframeSrc() {
	...
	const anime = parseAnime(getQueryAndFillForm(queryParams, 'anime'));
...
function parseAnime(anime) {
	...
	const regExpExecArray = ANIME_ID_REGEXP.exec(anime);
	if (!regExpExecArray) {
		const message = `Invalid anime [${anime}]`;
		postError(message);
		throw new Error(message)
	}
	return regExpExecArray.groups.id;
}
...
async function getPlayer(anime, translation) {
	const result = new URL('https://kodik-api.com/get-player');
...
function buildIframeUrl(iframeSrc, episode, season) {
	const result = new URL(iframeSrc.startsWith("//") ? `https:${iframeSrc}` : iframeSrc);
	const queryParams = new URLSearchParams();
